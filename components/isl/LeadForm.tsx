...
export function LeadForm({ tipo, propiedadId, agenteId, extraFields, hiddenFields = [], className }: LeadFormProps) {
  ...
  return (
    <form onSubmit={handleSubmit} className={["space-y-4", className].filter(Boolean).join(" ")}>
      ...
      {!isHidden("nombre") ? <label className="block space-y-1"><span className="text-sm text-isl-black">Nombre</span><input name="nombre" required className={inputClass} autoComplete="name" /></label> : null}
      ...
      <button type="submit" disabled={isSending} className="min-h-11 rounded-sm bg-isl-black px-5 py-3 text-xs font-medium uppercase tracking-[0.12em] text-isl-white disabled:opacity-50">{isSending ? "Enviando…" : "Enviar consulta"}</button>
